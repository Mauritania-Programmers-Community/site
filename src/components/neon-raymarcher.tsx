...
const deterministicRandom = (seed: number) => ((seed * GOLDEN_RATIO) % 1);
  ...
  baseOffsets: Array.from({ length: amount }, (_, i) => {
    ...
    return {
      ...
      posSpeed: new THREE.Vector3(
        1.0 + deterministicRandom(seed1) * 4,
        1.0 + deterministicRandom(seed1 + 1) * 3.5,
        0.5 + deterministicRandom(seed1 + 2) * 2.0
      ),
      rotSpeed: new THREE.Vector3(
        0.1 + deterministicRandom(seed2) * 1,
        0.1 + deterministicRandom(seed2 + 1) * 1,
        0.1 + deterministicRandom(seed2 + 2) * 1
      ),
      posPhase: new THREE.Vector3(
        t + deterministicRandom(seed3) * Math.PI * 3.0,
        t * 1.3 + deterministicRandom(seed3 + 1) * Math.PI * 3.0,
        t * 0.7 + deterministicRandom(seed3 + 2) * Math.PI * 3.0
      ),
      rotPhase: new THREE.Vector3(
        t * 0.5 + deterministicRandom(seed3 + 3) * Math.PI * 2.0,
        t * 0.8 + deterministicRandom(seed3 + 4) * Math.PI * 2.0,
        t * 1.1 + deterministicRandom(seed3 + 5) * Math.PI * 2.0
      )
    }
  })
})
